...
module.exports = class MarkdownFile {
  ...
  async write () {
    ...
    if (fs.existsSync(this.destinationFilePath)) {
      const existingHash = crypto.createHash('md5').update(fs.readFileSync(this.destinationFilePath, { encoding: 'utf8' })).digest('hex')
      const hash = crypto.createHash('md5').update(html).digest('hex')
      if (hash === existingHash) {
        console.log('unchanged', this.destinationFilePath, existingHash)
        return this.toJSON()
      }
    }
