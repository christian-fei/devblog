...
module.exports = class MarkdownFile {
  ...
  async toHTML ({ layout }) {
    let htmlContentWithLayout = this.htmlContent
    if (layout) {
      htmlContentWithLayout = `{% extends '${layout}' %}${this.htmlContent}`
    }
    ...
    return this.attributes.minify === false ? renderedContent : minify(renderedContent, {
